     ...
     <div class="page-main-header" :class="{ close_icon:!togglesidebar }">
      ...
      <div class="page-body-wrapper horizontal-menu">
        <div class="bg-overlay" :class="{active: activeoverlay }" @click="removeoverlay()"></div>
         <header class="main-nav" :class="[{ close_icon : !togglesidebar }]" :sidebar-layout="layout.settings.sidebar_setting">
...
<script>
 ...
 import Sidebar from './slidebar/indexVue.vue';
  ...
  components: { 
   ...
   data(){
    ...
    created(){
      ...
      this.layout.settings.layout = this.$route.query.layout? this.$route.query.layout : 'Defaul';
      this.layoutobj = layoutClasses.find((item) => Object.keys(item).pop() === this.layout.settings.layout);
      this.layoutobj = JSON.parse(JSON.stringify(this.layoutobj))[this.layout.settings.layout]; 
    },
    methods:{
